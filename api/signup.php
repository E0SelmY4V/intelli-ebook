...
$check_pw = function ($where) use ($end) {
	if (!($pw = $_POST['password_1'])) $end([['no_password', $where]]);
	if ($pw !== $_POST['password_2']) $end([['wrong_password', $where]]);
...
switch ($_POST['step']) {
	case 'email':
		...
		if (!filter_var($email, FILTER_VALIDATE_EMAIL)) $end(['wrong_email', $email]);
